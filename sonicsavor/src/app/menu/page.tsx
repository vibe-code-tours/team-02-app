    ...
    <main className="min-h-screen bg-[#0F0E17] pb-24">
      <div className="max-w-lg mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-[#FFFFFE]">Menu</h1>
          ...
          {cartCount > 0 && (
            <div className="relative">
              <svg
                className="w-6 h-6 text-[#FFFFFE]"
                viewBox="0 0 24 24"
                fill="currentColor"
              >
                <path d="M7 18c-1.1 0-1.99.9-1.99 2S5.9 22 7 22s2-.9 2-2-.9-2-2-2zM1 2v2h2l3.6 7.59-1.35 2.45c-.16.28-.25.61-.25.96 0 1.1.9 2 2 2h12v-2H7.42c-.14 0-.25-.11-.25-.25l.03-.12.9-1.63h7.45c.75 0 1.41-.41 1.75-1.03l3.58-6.49c.08-.14.12-.31.12-.48 0-.55-.45-1-1-1H5.21l-.94-2H1zm16 16c-1.1 0-1.99.9-1.99 2s.89 2 1.99 2 2-.9 2-2-.9-2-2-2z" />
              </svg>
              <span className="absolute -top-2 -right-2 w-5 h-5 bg-[#E85D04] text-[#FFFFFE] text-xs rounded-full flex items-center justify-center">
                {cartCount}
              </span>
            </div>
          )}
        ...
        {filteredItems.length === 0 && (
          <div className="text-center py-12">
            <svg
              className="w-16 h-16 mx-auto text-[#232946] mb-4"
              viewBox="0 0 24 24"
              fill="currentColor"
            >
              <path d="M11 15h2v2h-2zm0-8h2v6h-2zm.99-5C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8z" />
            </svg>
            <p className="text-[#94A1B2]">No items match your mood</p>
          </div>
        )}
      </div>
    </main>
